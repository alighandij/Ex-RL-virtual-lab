...
class PendulumEnv(gym.Env):
    ...
    | Num | Action | Discrete Value | Continuos Value |
    |-----|--------|----------------|-----------------|
    | 0   | Torque |         0      |       -2.0      |
    | 1   | Torque |         1      |       0.00      |
    | 2   | Torque |         2      |       +2.0      |
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def done(self):
        c, s, t = self._get_obs()
        # cond = c <= 0 and s >= 1
        c = np.arccos(c) * 180 / np.pi
        cond = c <= 100 and np.isclose(s, -1, atol=0.01)
    ...
    def step(self, u):
        ...
        l = self.l
        dt = self.dt
        u = [-self.max_torque, 0, +self.max_torque][u]
        ...
        self.episode_step += 1
        return self._get_obs(), -1 if self.r_1 else costs, self.done(), False, {}
